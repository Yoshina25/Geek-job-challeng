...
public class UpdateResult extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            /*セッションを受け取り、検索に使う引数にする。*/
            HttpSession UserID =request.getSession();
            UpData.setUserID(Integer.parseInt(UserID.getAttribute("userID").toString()));
            
            //ユーザー情報群をセッションに格納
            UserDataDAO.getInstance().update(UpData);
            System.out.println("Session updated!!");
            
            UserDataDTO resultData = UserDataDAO.getInstance().searchByID(UpData);
